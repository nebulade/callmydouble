...
function signoff(req, res, next) {
    ...
    verify(auth.user, auth.password, function (error, result) {
        ...
        var key = result.data[APPKEY_KEY];
        var secret = result.data[APPSECRET_KEY];
        var keyHash = hashKeyWithSecret(key, secret);

        store.del([keyHash, key, auth.user]);
...
function start(options, callback) {
    ...
    app.post('/api/v1/users/signoff', signoff);
