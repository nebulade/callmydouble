...
var SIMPLE_AUTH = process.env.SIMPLE_AUTH_URL && process.env.SIMPLE_AUTH_CLIENT_ID;
...
function verifyUserToken(userToken, callback) {
    if (SIMPLE_AUTH) {
        superagent.get(process.env.API_ORIGIN + '/api/v1/profile').query({ access_token: userToken }).end(function (error, result) {
            if (error && error.status === 401) return callback(null, { exists: false, valid: false, data: null });
            if (error) return callback(error);

            console.log('verifyUserToken', result.body)

            callback(null, { exists: true, valid: true, data: { user: result.body.username }});
        });
    } else {
...
function verify(user, password, callback) {
    if (SIMPLE_AUTH) {
        ...
        superagent.post(process.env.SIMPLE_AUTH_URL + '/api/v1/login').send(authPayload).end(function (error, result) {
            if (error && error.status === 401) return callback(null, { exists: false, valid: false, data: null });
            if (error) return callback(error);
            if (result.status !== 200) return callback(null, { exists: false, valid: false, data: null });
...
function generateAndStoreNewDetails(user, password, callback) {
    ...
    if (!SIMPLE_AUTH) {
        ...
        data.userToken = generateUserToken();

        store.set(data.userToken, user);
    }
...
function login(req, res) {
    ...
    verify(auth.user, auth.password, function (error, result) {
        ...
        if (SIMPLE_AUTH) {
            res.send(200, { userToken: result.data.dataccessToken });
        } else {
...
function signup(req, res) {
    if (SIMPLE_AUTH) return res.send(409, 'Use Cloudron user management');
