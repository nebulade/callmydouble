...
function usersSignin(req, res, next) {
    ...
    verify(auth.user, auth.password, function (error, result) {
        ...
        if (!result.exists) {
            ...
            userObj[APPKEY_KEY] = generateAppKey();
            userObj[APPSECRET_KEY] = generateAppSecret();

            store.set(auth.user, JSON.stringify(userObj), function (error) {
                if (error) {
                    debug('Failed to store user information', error);
                    return res.send(500);
                }
