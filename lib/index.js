...
function handleListener(socket) {
    ...
    socket.on('access_token', function (token) {
        ...
        store.hget(token, 'appKey', function (error, result) {
            if (error || !result) {
                debug('No application key available for this access token.');
                // TODO return an error?
                return;
            }

            appKey = result;
            debug('Client %s uses application key "%s".', token, appKey);
